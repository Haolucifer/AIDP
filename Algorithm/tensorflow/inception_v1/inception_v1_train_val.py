    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            ...
            if i % train_log_step == 0:
                ...
                datetime.now(), i, train_loss, train_acc), flush=True)

            # val测试(测试全部val数据)
            if i % val_log_step == 0:
                mean_loss, mean_acc = net_evaluation(sess, loss, accuracy, val_images_batch, val_labels_batch, val_nums)
                print("%s: Step [%d]  val Loss : %f, val accuracy :  %g" % (datetime.now(), i, mean_loss, mean_acc),flush=True)

            # 模型保存:每迭代snapshot次或者最后一次保存模型
            if (i % snapshot == 0 and i > 0) or i == max_steps:
                print('-----save:{}-{}'.format(snapshot_prefix, i), flush=True)
            ...
            if mean_acc > max_acc and mean_acc > 0.7:
                ...
                print('------save:{}'.format(best_models), flush=True)
...
if __name__ == '__main__':
    train_record_file='/root/inception_v1/dataset/record/train224.tfrecords'
    val_record_file='/root/inception_v1/dataset/record/val224.tfrecords'
