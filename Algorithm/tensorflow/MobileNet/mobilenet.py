...
parser = argparse.ArgumentParser(description='the implement of inception_v1 with tensorflow')
parser.add_argument('-dp','--dataset_path',default='./dataset/',
                    type=str,help='the path of your original train dataset')
parser.add_argument('-tlp','--train_label_path',default='./dataset/train/train.csv',
                    type=str, help='the path of generated train csv')
parser.add_argument('-vlp','--val_label_path',default='./dataset/val/val.csv',
                    type=str, help='the path of generated val csv')
parser.add_argument('-t','--image_type',default='.jpg')
parser.add_argument('-g','--gpu_id',default='0', type=str)
parser.add_argument('-smp','--saved_model_path',default='./models_modilenet/',type=str)
parser.add_argument('--log_step',default=100,type=int)
parser.add_argument('--base_lr',default=0.01,type=int)
parser.add_argument('--max_steps', default=10000, type=int)
parser.add_argument('--snapshot',default=2000, type=int, help='the frequency of saving the latest model')
...
_, _, labels_nums = create_csv_files.get_number_of_classification(args.dataset_path+'train/')
print(labels_nums)
...
#             print(batch_input_images)
#             print(batch_input_labels)
...
def train(data_csv_path_train,
    ...
    train_batch = data_loader.load_data(data_csv_path_train,image_type=args.image_type,
...
#     print(train_labels_batch)
    ...
    # val数据,验证数据可以不需要打乱数据
    val_batch = data_loader.load_data(data_csv_path_val, image_type=args.image_type, image_size_before_crop=resize_height, labels_nums=labels_nums,do_shuffle=False)
    ...
    with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
        out, end_points = mobilenet_v1.mobilenet_v1(inputs=input_images, num_classes=labels_nums, dropout_keep_prob=keep_prob, is_training=is_training,global_pool=True)
...
if __name__ == '__main__':
    dataset_csv_path_train = args.train_label_path
    dataset_csv_path_val = args.val_label_path

    # 判断是否已生成csv文件
    if not os.path.exists(dataset_csv_path_train):
        train_source_path = args.dataset_path + 'train/'
        create_csv_files.write_to_csv(train_source_path, dataset_csv_path_train)
    if not os.path.exists(dataset_csv_path_val):
        val_source_path = args.dataset_path + 'val/'
        create_csv_files.write_to_csv(val_source_path, dataset_csv_path_val)

    train_log_step = args.log_step
    base_lr = args.base_lr  # 学习率
    max_steps = args.max_steps  # 迭代次数
    train_param = [base_lr, max_steps]

    val_log_step = args.log_step
    snapshot = args.snapshot  # 保存文件间隔
    if not os.path.exists(args.saved_model_path):
        os.mkdir(args.saved_model_path)
    snapshot_prefix = args.saved_model_path+'model.ckpt'
    train(data_csv_path_train=dataset_csv_path_train,
          train_log_step=train_log_step,
          train_param=train_param,
          data_csv_path_val=dataset_csv_path_val,
